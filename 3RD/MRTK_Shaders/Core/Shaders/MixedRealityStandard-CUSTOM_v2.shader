...
//FIXED THE METALLIC AND SMOOTHNESS FOR NO CHANNEL_MAP BY INITIALIZING IT TO BLACK AND USING AND IF()
//DEACTIVATED DIRECTIONAL LIGHT AND EMISSION CONTRIBUTIONS FOR LIGHTMAP ON VERSIONS OF THE SHADER (check all emission but all deactivated)

Shader "Mixed Reality Toolkit/Standard-CUSTOM_v2"
{
    Properties
    {
        // Main maps.
        ...
        [NoScaleOffset] _ChannelMap("Channel Map", 2D) = "black" {}
    ...
    SubShader
    {
        Pass
        {
            ...
            #pragma multi_compile _ _METALLIC_TEXTURE_ALBEDO_CHANNEL_A _SMOOTHNESS_TEXTURE_ALBEDO_CHANNEL_A
            #pragma multi_compile _CHANNEL_MAP
            #pragma multi_compile _NORMAL_MAP
            ...
            #pragma multi_compile _REFLECTIONS
...
#define PROXIMITY_LIGHT_DATA_SIZE 6
            ...
            // Texturing.
				...
				_Metallic = IF(channel.r > 0, channel.r, _Metallic);				
				//albedo.rgb *= channel.g;
				_Smoothness = IF(channel.a > 0, channel.a, _Smoothness);
#else
