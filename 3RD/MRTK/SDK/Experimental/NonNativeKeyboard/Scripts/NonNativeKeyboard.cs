...
namespace Microsoft.MixedReality.Toolkit.Experimental.UI
{
    ...
    public class NonNativeKeyboard : MonoBehaviour
    {
        ...
        public void PresentKeyboard(string startText)
        {
            ...
            transform.position= Camera.main.transform.position + Camera.main.transform.forward * 1.2f - Vector3.up * 0.3f; 
            PresentKeyboard();
            Clear();
        ...
        void FollowHead()
        {
            var followAnchor = Camera.main.transform.position + Camera.main.transform.forward*1.2f - Vector3.up * 0.3f;
            if (Vector3.Distance(followAnchor, transform.position) > 1f && !lerpingKeyboard)
            {
                lerpingKeyboard = true;
            }

            if (lerpingKeyboard)
            {
                transform.position = Vector3.Lerp(transform.position, followAnchor, Time.deltaTime);
            }

            if(Vector3.Distance(followAnchor, transform.position) < 0.05f)
            {
                lerpingKeyboard = false;
            }
